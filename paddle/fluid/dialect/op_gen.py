...
INPUT_TYPE_CHECK_TEMPLATE = """PADDLE_ENFORCE_EQ(inputs[{index}].type().isa<{standard}>(), true,
  ...
  """
INPUT_VECTORTYPE_CHECK_TEMPLATE = """PADDLE_ENFORCE_EQ(inputs[{index}].type().isa<ir::VectorType>(), true,
                    phi::errors::PreconditionNotMet("Type validation failed for the {index}th input."));
  PADDLE_ENFORCE_EQ(inputs[{index}].type().dyn_cast<ir::VectorType>().element_type().isa<{standard}>(), true,
                    phi::errors::PreconditionNotMet("Type validation failed for the {index}th input."));
  """
...
INPUT_OPTIONAL_VECTORTYPE_CHECK_TEMPLATE = """if (inputs[{index}]) {{
    PADDLE_ENFORCE_EQ(inputs[{index}].type().isa<ir::VectorType>(), true,
                      phi::errors::PreconditionNotMet("Type validation failed for the {index}th input."));
    PADDLE_ENFORCE_EQ(inputs[{index}].type().dyn_cast<ir::VectorType>().element_type().isa<{standard}>(), true,
                      phi::errors::PreconditionNotMet("Type validation failed for the {index}th input."));
  }}
...
OUTPUT_OPTIONAL_VECTORTYPE_CHECK_TEMPLATE = """if (outputs[{index}]) {{
    PADDLE_ENFORCE_EQ(outputs[{index}].isa<ir::VectorType>(), true,
                      phi::errors::PreconditionNotMet("Type validation failed for the {index}th output."));
    PADDLE_ENFORCE_EQ(outputs[{index}].dyn_cast<ir::VectorType>().element_type().isa<{standard}>(), true,
                      phi::errors::PreconditionNotMet("Type validation failed for the {index}th output."));
  }}
